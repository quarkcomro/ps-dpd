...
class DpdGeopostConfigurationController extends DpdGeopostController
{
    ...
    const CACHE_CLEAR_ACTION = 'dpdApiCacheClear';
    ...
    public function getSettingsPage()
    {
        ...
        require_once(_DPDGEOPOST_CLASSES_DIR_ . 'apiCache.php');

        $configuration = new DpdGeopostConfiguration();
        $apiCacheStats = DpdGeopostApiCache::stats();
        $apiCacheEnabled = DpdGeopostApiCache::isEnabled();
        // Prune stale debug logs on settings page load so freshly-saved retention values
        // take effect without waiting for the next API call.
        DpdGeopostApiDebugLogger::prune();
        $debugLogRetentionDays = DpdGeopostApiDebugLogger::getRetentionDays();
        ...
        $this->context->smarty->assign(array(
            ...
            'apiCacheEntries' => (int) $apiCacheStats['entries'],
            'apiCacheBytes' => (int) $apiCacheStats['bytes'],
            'apiCacheKb' => number_format($apiCacheStats['bytes'] / 1024, 1, '.', ''),
    ...
    public static function init()
    {
        ...
        if (Tools::isSubmit(self::CACHE_CLEAR_ACTION)) {
    ...
    {
        require_once(_DPDGEOPOST_CLASSES_DIR_ . 'apiCache.php');
        $removed = (int) DpdGeopostApiCache::clear();
        DpdGeopost::addFlashMessage(sprintf($this->l('DPD API cache cleared (%d entries removed).'), $removed));
    ...
    private function createDeleteCarriers()
    {
        ...
        if (Tools::getValue(DpdGeopostConfiguration::SERVICE_STANDARD_LOCKER)) {
            if (!DpdGeopostCarrierStandardLockerService::install()) {
                self::$errors[] = $this->l('Could not save DPD OOH Locker/Office/PUDO');
            }
        } else {
            if (!DpdGeopostCarrierStandardLockerService::delete()) {
                self::$errors[] = $this->l('Could not delete DPD OOH Locker/Office/PUDO');
            }
        }
    }

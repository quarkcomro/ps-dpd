...
class DpdGeopostCarrierStandardLockerService extends DpdGeopostService
{
	...
	public static function install()
	{
		...
		$carrier = new Carrier();
		$carrier->name = $carrier_service->module_instance->l('DPD OOH Locker/Office/PUDO', self::FILENAME);
		$carrier->active = 1;
		$carrier->is_free = 0;
		...
		foreach (Language::getLanguages(false) as $language)
			$delay[$language['id_lang']] = $carrier_service->module_instance->l('DPD OOH Locker/Office/PUDO', self::FILENAME);
		$carrier->delay = $delay;
		...
		foreach ($carrier_service->continents as $continent => $value) {

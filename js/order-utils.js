...
//    convert_address_to_str_nr_bl();
    ...
    var selectedCity = $('input[name="city"]').val();    filterPickupOffice();

        $(document).on('blur change', 'input[name="city"], select[name="id_country"]', function(e){
        filterPickupOffice();
    });

    function filterPickupOffice() {
        var officeSelect = $('select[name="dpd_office"]');
        var cityField = $('input[name="city"]');
        var countryField = $('select[name="id_country"]');
        var currentSelection = officeSelect.val();
        var selectedCity = $.trim(cityField.val() || '');
        var selectedCountry = $.trim(countryField.val() || '');

        if (!window._DPDGEOPOST_AJAX_URI_ || officeSelect.length === 0 || cityField.length === 0 || countryField.length === 0) {
            return;
        }

        if (!selectedCity || !selectedCountry || selectedCity === 'undefined' || selectedCountry === 'undefined') {
            officeSelect.attr('disabled', 'disabled');
            return;
        }

        var url = window._DPDGEOPOST_AJAX_URI_ + '?filterCity=' + encodeURIComponent(selectedCity) + '&filterCountry=' + encodeURIComponent(selectedCountry);
        officeSelect.attr('disabled', 'disabled');
        officeSelect.html('...');
        ...
        $.getJSON(url, function (response) {
            if(response.empty) {
                officeSelect.attr('disabled', 'disabled');
                officeSelect.html(response.html);
            } else {
                officeSelect.html(response.html);
                officeSelect.removeAttr('disabled');
                setTimeout(function () {
                    officeSelect.val(currentSelection);
                }, 250);
            }

$(document).ready(function(){
	...
	$('#displayPickupDialog').click(function(){
		...
		if(selected_shipments)
		{
			...
			if (available_pickup)
			{
				// Wipe any error message left over from a previous attempt so reopening
				// the dialog after a failed submit (e.g. locality_not_served) presents
				// a clean form rather than the stale red banner.
				$('#dpdgeopost_pickup_dialog_mssg').stop(true, true).hide().empty();
	...
	$(document).on('click', '.dpd-close-pickup-dialog', function(){
		$('#dpdgeopost_pickup_dialog').bPopup().close();
	});

	$('#submit_dpdgeopost_pickup_dialog').click(function(){
		var $btn = $(this);
		// Guard: ignore re-clicks while a submit is already in flight.
		if ($btn.prop('disabled')) {
			return;
		}
		// Stash the button's natural label so we can restore it after the AJAX completes
		// (success-with-error or transport error). On success the page reloads, so the
		// restored state is never seen — but it's still cheap insurance.
		var originalBtnHtml = $btn.html();
		var loadingLabel = (typeof dpdgeopost_label_pickup_submitting === 'string' && dpdgeopost_label_pickup_submitting)
			? dpdgeopost_label_pickup_submitting
			: 'Submitting…';
		$btn.prop('disabled', true).html('<i class="icon-refresh icon-spin"></i> ' + loadingLabel);
		...
		$.ajax({
			...
			complete: function()
			{
				// Runs after both success and error paths. Re-enable the button so the
				// user can retry after a server-side rejection (e.g. locality_not_served).
...
{
	var ok = true;
	$('input[name="shipmentBox[]"]:checked').each(function(){
		var pickupAvailable = $(this).closest('tr').find('input[name="pickup_available"]').val();
		if (pickupAvailable == '0' || typeof pickupAvailable === 'undefined')
			ok = false;
	});
	return ok;
}

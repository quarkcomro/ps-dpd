...
if(Tools::isSubmit('filterCity')) {
	$filterCity = trim((string) Tools::getValue('filterCity'));
	$filterCountry = trim((string) Tools::getValue('filterCountry'));

    $invalidFilterCity = $filterCity === '' || Tools::strtolower($filterCity) === 'undefined';
    $invalidFilterCountry = $filterCountry === '' || Tools::strtolower($filterCountry) === 'undefined';
    if ($invalidFilterCity || $invalidFilterCountry) {
        echo json_encode(array(
            'html' => "<option > Nu am gasit oficii in orasul introdus </option> ",
            'empty' => true,
        ));
        exit;
    }

	...
	<form id="defaultForm form-horizontal" class="defaultForm" action="{$saveAction|escape:'htmlall':'UTF-8'}&menu=configuration"
		...
		<div class="panel" id="fieldset_debug">
			<div class="panel-heading">
				<i class="icon-bug"></i>{l s='Debug' mod='dpdgeopost'}
			</div>

			<div class="form-wrapper">
				<div class="form-group">
					<div class="checkbox">
						<label for="dpd_debug_mode">
							<input id="dpd_debug_mode" type="checkbox" name="{DpdGeopostConfiguration::DEBUG_MODE|escape:'htmlall':'UTF-8'}" value="1"
								{if DPDGeopost::getInputValue(DpdGeopostConfiguration::DEBUG_MODE, $settings->debug_mode) == 1}checked="checked"{/if} />
							{l s='Enable API debug logging' mod='dpdgeopost'}
						</label>
						<input type="hidden" name="{DpdGeopostConfiguration::DEBUG_FILE|escape:'htmlall':'UTF-8'}" value="{DPDGeopost::getInputValue(DpdGeopostConfiguration::DEBUG_FILE, $settings->debug_file)|escape:'htmlall':'UTF-8'}" />
						<small class="form-text text-muted">
							{l s='When enabled, every DPD API request and response is recorded with response time into daily log files.' mod='dpdgeopost'}
						</small>
					</div>
				</div>

				<div class="form-group">
					<label for="dpd_debug_log_retention_days">
						{l s='Log retention (days)' mod='dpdgeopost'}
					</label>
					<input id="dpd_debug_log_retention_days" type="number" min="0" max="365"
						   name="{DpdGeopostConfiguration::DEBUG_LOG_RETENTION_DAYS|escape:'htmlall':'UTF-8'}"
						   value="{$debugLogRetentionDays|intval}"
						   class="form-control" />
					<small class="form-text text-muted">
						{l s='Debug log files older than this many days are deleted automatically. Set to 0 to keep them indefinitely. Default: 5.' mod='dpdgeopost'}
					</small>
				</div>
			</div>
		...
		<div class="panel" id="fieldset_api_cache">
			<div class="panel-heading">
				<i class="icon-bolt"></i>{l s='API cache' mod='dpdgeopost'}
			</div>

			<div class="form-wrapper">
				<div class="form-group">
					<div class="checkbox">
						<label for="dpd_api_cache_enabled">
							<input id="dpd_api_cache_enabled" type="checkbox"
								   name="{DpdGeopostConfiguration::API_CACHE_ENABLED|escape:'htmlall':'UTF-8'}" value="1"
								   {if DPDGeopost::getInputValue(DpdGeopostConfiguration::API_CACHE_ENABLED, $settings->api_cache_enabled) == 1}checked="checked"{/if} />
							{l s='Enable DPD API response cache' mod='dpdgeopost'}
						</label>
						<small class="form-text text-muted">
							{l s='When enabled, safe read-only DPD API responses (countries, sites, streets, offices, shipping-cost calculations) are cached on disk with per-endpoint TTLs. Stateful endpoints (shipment, track, pickup) are never cached.' mod='dpdgeopost'}
						</small>
					</div>
				</div>

				<div class="form-group">
					<label>{l s='Current cache size' mod='dpdgeopost'}</label>
					<div>
						{if $apiCacheEnabled}
							<span class="label label-success">{l s='Active' mod='dpdgeopost'}</span>
						{else}
							<span class="label label-default">{l s='Disabled' mod='dpdgeopost'}</span>
						{/if}
						&nbsp;
						{$apiCacheEntries|intval} {l s='entries' mod='dpdgeopost'}
						&middot;
						{$apiCacheKb|escape:'htmlall':'UTF-8'} KB
					</div>
				</div>
			</div>
			...
			<div class="margin-form">
				...
				<button type="submit" class="btn btn-default"
						name="{DpdGeopostConfigurationController::CACHE_CLEAR_ACTION|escape:'htmlall':'UTF-8'}"
						value="1"
						onclick="return confirm('Clear all DPD API cache entries?');">
					<i class="icon-trash"></i> {l s='Clear DPD API cache' mod='dpdgeopost'}
				</button>
			</div>
		</div>

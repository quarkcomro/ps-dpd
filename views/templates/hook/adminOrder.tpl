<script>
    ...
    // Toggle for the DPD address-validation panel. The .dpd-normalizer block is
    // hidden by default (see template) so the order page stays compact for the
    // ~95% of orders that don't need re-validation. Merchant clicks the button
    // when an address issue needs fixing. Caret rotates 180° to give a "expanded"
    // visual cue. We don't use slideToggle on initial state because the panel
    // contains select2 widgets that need the DOM visible to size correctly —
    // slideToggle works fine for collapse/expand AFTER the first reveal though.
    $(document).ready(function () {
        $(document).on('click', '.js-toggle-dpd-normalizer', function () {
            var $btn = $(this);
            var $panel = $('.dpd-normalizer');
            var expanded = $btn.attr('aria-expanded') === 'true';
            if (expanded) {
                $panel.slideUp(150);
                $btn.attr('aria-expanded', 'false');
                $btn.find('.js-toggle-caret').css('transform', 'rotate(0deg)');
            } else {
                $panel.slideDown(150);
                $btn.attr('aria-expanded', 'true');
                $btn.find('.js-toggle-caret').css('transform', 'rotate(180deg)');
            }
        });
        // Sync caret with initial aria-expanded state (auto-expand case).
        $('.js-toggle-dpd-normalizer').each(function () {
            if ($(this).attr('aria-expanded') === 'true') {
                $(this).find('.js-toggle-caret').css('transform', 'rotate(180deg)');
            }
        });
    });
</script>
...
<fieldset id="dpdgeopost">
    ...
    {if $settings->checkRequiredFields()}
        ...
        <br/>
                ...
                {$deliveryAddress->city|escape:'htmlall':'UTF-8'}{if !empty($deliveryAddressStateName)}
                {$deliveryAddressStateName|escape:'htmlall':'UTF-8'}{/if}
        ...
        {*
            DPD address-validation form is hidden by default — most orders don't need
            re-validation, the merchant only opens it when an address issue needs fixing.
            Auto-expand when something already looks wrong (city / street ambiguity,
            address-line over the carrier's character limit) so problems aren't hidden.
        *}
        {assign var='dpdAutoExpand' value=(isset($street_error) || isset($city_error) || (isset($streetLengthErrors) && $streetLengthErrors))}
        <div style="margin: 10px 0;">
            <button type="button" class="btn btn-default btn-sm js-toggle-dpd-normalizer" aria-expanded="{if $dpdAutoExpand}true{else}false{/if}">
                <i class="icon-edit"></i> {l s='Validate / edit DPD address' mod='dpdgeopost'}
                <i class="icon-caret-down js-toggle-caret"></i>
            </button>
        </div>

        <div class="dpd-normalizer" {if !$dpdAutoExpand}style="display:none;"{/if}>
                        ...
                        <div class="form-group row ">
                            ...
                            <div class="col-md-10">
                                <select class="form-control dpd_confirm_region" type="text" style="width: 500px">
                                    <option value="" {if empty($deliveryAddressStateName)}selected="selected"{/if}></option>
                                    {foreach $states as $state }
                                        {if $deliveryAddressStateName == $state || $deliveryAddress->dpd_state == $state}
                ...
                <td>
                    ...
                    $shipment->id_manifest} disabled="disabled"{/if}>
                        ...
                        {if $settings->active_services_standard_24 }
                            <option value="{$smarty.const._DPDGEOPOST_STANDARD_24_ID_}" {if $selected_shipping_method_id == $smarty.const._DPDGEOPOST_STANDARD_24_ID_ && (empty($selectedOfficeId) || !$settings->active_services_locker)} selected="selected" {/if}>
                                DPD Standard 24
                            </option>
                        {/if}
        ...
        {if $shipment->id_shipment}
            <div class="dpdgeopost_shipment_action_buttons" class="container-sm">
                <a href="{$dpd_geopost_pdf_uri|escape:'htmlall':'UTF-8'}?printLabels=true&token={$dpd_geopost_token|escape:'htmlall':'UTF-8'}&id_shop={$dpd_geopost_id_shop|escape:'htmlall':'UTF-8'}&id_lang={$dpd_geopost_id_lang|escape:'htmlall':'UTF-8'}&id_order={$order->id|escape:'htmlall':'UTF-8'}"
                   id="dpdgeopost_print_labels" class="btn btn-secondary">
                   {l s='Print labels' mod='dpdgeopost'}
                </a>
                {if $hasVouchers}
                    <a href="{$dpd_geopost_pdf_uri|escape:'htmlall':'UTF-8'}?printVouchers=true&token={$dpd_geopost_token|escape:'htmlall':'UTF-8'}&id_shop={$dpd_geopost_id_shop|escape:'htmlall':'UTF-8'}&id_lang={$dpd_geopost_id_lang|escape:'htmlall':'UTF-8'}&id_order={$order->id|escape:'htmlall':'UTF-8'}"

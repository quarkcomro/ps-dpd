...
class DpdGeopostConfiguration extends DpdGeopostObjectModel
{
	...
	const DEBUG_LOG_RETENTION_DAYS					= 'DPD_GEOPOST_DEBUG_LOG_RETENTION_DAYS';
	const DEBUG_LOG_LAST_PRUNED						= 'DPD_GEOPOST_DEBUG_LOG_LAST_PRUNED';
	const ADDRESS_VALIDATION 						= 'DPD_GEOPOST_ADDRESS_VALIDATION';
	const API_CACHE_ENABLED							= 'DPD_GEOPOST_API_CACHE_ENABLED';
	...
	public $debug_log_retention_days		= 5;
	public $address_validation				= 0;
	public $api_cache_enabled				= 1;
	...
	public static function saveConfiguration()
	{
		...
		$retentionDays = (int) Tools::getValue(self::DEBUG_LOG_RETENTION_DAYS);
		if ($retentionDays < 0) {
			$retentionDays = 0;
		} elseif ($retentionDays > 365) {
			$retentionDays = 365;
		}
		$success &= Configuration::updateValue(self::DEBUG_LOG_RETENTION_DAYS,				$retentionDays);
		$success &= Configuration::updateValue(self::ADDRESS_VALIDATION, 					(int)Tools::getValue(self::ADDRESS_VALIDATION));
		$success &= Configuration::updateValue(self::API_CACHE_ENABLED, 					(int)Tools::getValue(self::API_CACHE_ENABLED));
...
/*		$success &= Configuration::updateValue(self::SERVICE_CLASSIC, 						(int)Tools::getValue(self::SERVICE_CLASSIC));
	...
	private function getSettings()
	{
		...
		$this->debug_log_retention_days			= (int) $this->getSetting(self::DEBUG_LOG_RETENTION_DAYS,		$this->debug_log_retention_days);
		$this->address_validation				= $this->getSetting(self::ADDRESS_VALIDATION, 					$this->address_validation);
		$this->api_cache_enabled				= $this->getSetting(self::API_CACHE_ENABLED,					$this->api_cache_enabled);
	...
	public static function deleteConfiguration()
	{
		...
		Configuration::deleteByName(self::API_CACHE_ENABLED);
		Configuration::deleteByName(self::DEBUG_LOG_RETENTION_DAYS);
		Configuration::deleteByName(self::DEBUG_LOG_LAST_PRUNED);

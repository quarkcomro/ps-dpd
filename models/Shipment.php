...
class DpdGeopostShipment extends DpdGeopostWs
{
    ...
    private $data = array();
    public $parcels;
    private static $countryLookupCache = array();
    private static $siteLookupCache = array();
    private static $calculateResultCache = array();
    private static $shipmentInfoCache = array();
    ...
    public function __construct($id_order = null, $shipment = null)
    {
        ...
        if ($this->id_shipment) {
            $result = $this->fetchShipmentInfo(array($this->id_shipment));
            if (isset($result['shipmentResultList']) && isset($result['shipmentResultList']['shipment']))
                $this->data = $result['shipmentResultList']['shipment'];


            $this->parcels = isset($result['shipments'][0]['content']['parcels'])
                ? $result['shipments'][0]['content']['parcels']
                : null;
        }
    ...
    public function calculatePriceForOrder($id_method, $id_address, $products = array(), $extra_params = array(), $withCurrency = true)
    {
        ...
        if ($result = $this->calculate($id_method, $id_address, $parcels, $order, $extra_params)) {
            ...
            if ($withCurrency) {
                $currency = Currency::getCurrencyInstance((int) $result['id_currency']);
                $currencyIsoCode = Validate::isLoadedObject($currency)
                    ? $currency->iso_code
                    : Currency::getIsoCodeById((int) $result['id_currency']);

                return Tools::getContextLocale($this->context)->formatPrice($result['price'], $currencyIsoCode);
            }
    ...
    public function calculate($id_method, $id_address, $parcels, Order $order = null, $extra_params = array())
    {
        ...
        $countryWs = $this->getCountryWsByIsoCode($country->iso_code);
        ...
        if (isset($params['recipient']['addressLocation']) && isset($params['recipient']['addressLocation']['siteName'])) {
            ...
            if (Validate::isLoadedObject($countryState)) {
                $findSiteRequest['region'] = $countryState->name;
            }

            $locationsFound = $this->getLocationsForRequest($findSiteRequest);
        ...
        $calculatePayloadCacheKey = $this->buildPayloadCacheKey($params);
        if (isset(self::$calculateResultCache[$calculatePayloadCacheKey])) {
            $this->logPayloadCacheEvent('hit', 'calculate', $params, $calculatePayloadCacheKey, __METHOD__);
            return self::$calculateResultCache[$calculatePayloadCacheKey];
        }
        ...
        if (!reset(self::$errors)) {
            ...
            if ($id_currency = Currency::getIdByIsoCode($result['calculations'][0]['price']['currencyLocal'], $this->context->shop->id)) {
                ...
                self::$calculateResultCache[$calculatePayloadCacheKey] = $calculatedResult;
                $this->logPayloadCacheEvent('store', 'calculate', $params, $calculatePayloadCacheKey, __METHOD__);
    ...
    private function getCountryWsByIsoCode($isoCode)
    {
        $cacheKey = strtoupper((string) $isoCode);
        if (isset(self::$countryLookupCache[$cacheKey])) {
            return self::$countryLookupCache[$cacheKey];
        }

        $dpdAddressSearch = new DpdAddressSearch();
        $countryWs = $dpdAddressSearch->getCountryByIsoCode($isoCode);
        self::$countryLookupCache[$cacheKey] = $countryWs;

        return $countryWs;
    }

    private function fetchShipmentInfo(array $shipmentIds)
    {
        ...
        sort($shipmentIds);
        $cacheKey = sha1(implode(',', $shipmentIds));
        if (array_key_exists($cacheKey, self::$shipmentInfoCache)) {
            $this->logPayloadCacheEvent('hit', 'shipment_info', array('shipmentIds' => $shipmentIds), $cacheKey, __METHOD__);
            return self::$shipmentInfoCache[$cacheKey];
        }

        $response = $this->wsrest_shipment_info(array('shipmentIds' => $shipmentIds));
        self::$shipmentInfoCache[$cacheKey] = is_array($response) ? $response : array();
        $this->logPayloadCacheEvent('store', 'shipment_info', array('shipmentIds' => $shipmentIds), $cacheKey, __METHOD__);

        return self::$shipmentInfoCache[$cacheKey];
    }

    private function getLocationsForRequest(array $findSiteRequest)
    {
        $cacheKey = $this->buildPayloadCacheKey($findSiteRequest);
        if (isset(self::$siteLookupCache[$cacheKey])) {
            $this->logPayloadCacheEvent('hit', 'location_site', $findSiteRequest, $cacheKey, __METHOD__);
            return self::$siteLookupCache[$cacheKey];
        }

        $locationsFound = $this->wsrest_location_site($findSiteRequest);
        self::$siteLookupCache[$cacheKey] = $locationsFound;
        $this->logPayloadCacheEvent('store', 'location_site', $findSiteRequest, $cacheKey, __METHOD__);

        return $locationsFound;
    }

    private function buildPayloadCacheKey(array $payload)
    {
        $normalizedPayload = $this->normalizePayloadForCache($payload);

        return sha1(json_encode($normalizedPayload, JSON_UNESCAPED_SLASHES));
    }

    private function normalizePayloadForCache($value)
    {
        if (!is_array($value)) {
            return $value;
        }
        ...
        foreach ($value as $key => $childValue) {
            $value[$key] = $this->normalizePayloadForCache($childValue);
        }
    ...
    private function logPayloadCacheEvent($operation, $method, array $payload, $cacheKey, $source)
    {
        DpdGeopostApiDebugLogger::writeCacheEvent($operation, $method, $payload, $cacheKey, array(
...
//        }
    ...
    public function getLabelsPdf($shipment_ids = null)
    {
        ...
        foreach ($shipment_ids as $id_shipment) {
            ...
            if (!$shipment->parcels) {
                ...
                $result = $this->fetchShipmentInfo(array($this->id_shipment));
                $ws_parcels = isset($result['shipments'][0]['content']['parcels'])
                    ? $result['shipments'][0]['content']['parcels']
                    : array();
            } else {
    ...
    function addTrackingNumber(&$order, $tracking_number, $id_carrier, $id_address)
    {
        ...
        if (!Validate::isLoadedObject($order_carrier)) {
            ...
            if ($order_carrier->update()) {
                ...
                $mailShopId = $this->resolveMailShopId($order);
                if (@Mail::Send(
                    (int)$order->id_lang,
                    'in_transit',
    ...
    private function resolveMailShopId(Order $order)
    {
        $shopId = (int) $order->id_shop;

        if ($shopId <= 0 && isset($this->context->shop->id)) {
    ...
    {
        $response = $this->fetchShipmentInfo((array) $ids);
